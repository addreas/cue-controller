...
func TestMain(m *testing.M) {
	...
	utilruntime.Must(cuebuildv1.AddToScheme(scheme.Scheme))
	...
	controllerName := "cuebuild-controller"
	testEventsH = controller.MakeEvents(testEnv, controllerName, nil)
	testMetricsH = controller.MustMakeMetrics(testEnv)
	reconciler := &CueBuildReconciler{
		ControllerName:  controllerName,
		Client:          testEnv,
		EventRecorder:   testEventsH.EventRecorder,
		MetricsRecorder: testMetricsH.MetricsRecorder,
	}
	if err := (reconciler).SetupWithManager(testEnv, CueBuildReconcilerOptions{MaxConcurrentReconciles: 4}); err != nil {
