...
func (r *CueBuildReconciler) SetupWithManager(mgr ctrl.Manager, opts CueBuildReconcilerOptions) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			handler.EnqueueRequestsFromMapFunc(r.requestsForRevisionChangeOf(cuebuildv1.GitRepositoryIndexKey)),
			builder.WithPredicates(SourceRevisionChangePredicate{}),
		).
		Watches(
			&source.Kind{Type: &sourcev1.Bucket{}},
			handler.EnqueueRequestsFromMapFunc(r.requestsForRevisionChangeOf(cuebuildv1.BucketIndexKey)),
			builder.WithPredicates(SourceRevisionChangePredicate{}),
		).
...
func (r *CueBuildReconciler) reconcile(
	...
	checksum := source.GetArtifact().Revision[strings.Index(source.GetArtifact().Revision, "/")+1:]

	// build the cueBuild and generate the GC snapshot
	resources, err := r.build(ctx, cueBuild, values, checksum)
	if err != nil {
		return cuebuildv1.CueBuildNotReady(
			cueBuild,
			source.GetArtifact().Revision,
	...
	snapshot, err := cuebuildv1.NewSnapshot(resources, checksum)
	if err != nil {
		return cuebuildv1.CueBuildNotReady(
			cueBuild,
			source.GetArtifact().Revision,
...
func (r *CueBuildReconciler) build(ctx context.Context, cueBuild cuebuildv1.CueBuild, values []cue.Value, checksum string) ([]byte, error) {
	...
	labels := map[string]string{
		"cuebuild.toolkit.fluxcd.io/checksum":  checksum,
		"cuebuild.toolkit.fluxcd.io/name":      cueBuild.Name,
		"cuebuild.toolkit.fluxcd.io/namespace": cueBuild.Namespace,
	}
